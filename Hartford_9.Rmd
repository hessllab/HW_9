...
```
The structure consists of a data frame with 192 obervations of 216 variables. The country vector is listed as 'Factor', but can be easily corrected to 'Character' by implementing the argument 'stringsAsFactors = FALSE".
```
...
cntrys <- C
dat <- NA
ex_tab <- data.frame("Country"=cntrys, "PerCapEmissions_2014"=dat)
ex_tab %>%
  kable() %>%
  kable_styling()
```
